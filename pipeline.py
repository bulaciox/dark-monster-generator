...
# The monster screen's occasional animation is generated in the background,
# AFTER a monster's four synchronous outputs are already saved and shown to
# the visitor -- their wait must not grow to cover video generation too. A
# small, separate pool keeps it from competing with the per-request executor
...
def _process_submission(sub: dict) -> tuple[dict, str]:
    ...
    # Fire-and-forget: the visitor already has their result. This does not
...
def _silhouette_video(monster_id: str, silhouette_image_url: str) -> None:
    """Background job: generate the monster screen's animation clip and
    attach it once ready. Runs after the visitor already has their result, so
    failures here are logged and swallowed rather than surfaced anywhere.
    """
    video_url = _retry(
        lambda: store_video(generate_silhouette_video(silhouette_image_url)),
        "silhouette video generation")
    if video_url:
        save_silhouette_video(monster_id, video_url)
